...
for j in range(n_data):
    for i in range(1,n_groups):
        for k in range(100):
            WR_percap[j][k,i] = 1 - math.exp(-(p*q*times_erl[j][k,i]/Q))
#############################################################
...
for j in range(n_data):
    for i in range(1,n_groups):# we avoid i=1 since this doesnt result in distribtuion since only party size of 1. start at Party size = 2
        erl_percap[j,i] = 1- ((lam_erl[j,i]/((p*q/Q)+lam_erl[j,i]))**k_erl[j,i])
##########################################################
...
for i in range(n_data):
    ...
    plt.scatter([0,1,2,3],erl_percap[i,1:], marker='o', color='k', label='Erlang Per Capita P(T) (Equation (27))')
    plt.legend()
    plt.show()
